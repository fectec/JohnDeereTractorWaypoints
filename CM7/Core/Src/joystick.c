...
// Global array to store joystick input data
...
{
	// Declare a pointer for tokenization and create a modifiable copy of the input data
	...
	// Loop through the tokens until there are no more or the index exceeds JOYSTICK_INPUTS

	while (token != NULL && index < JOYSTICK_INPUTS)
	{
		// Convert the token to an integer and store it in the joystick_data array
	...
	// Free the memory allocated for the duplicated string to prevent memory leaks
